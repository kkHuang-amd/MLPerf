...
bool dispatch_softmax(output_t *dst, const input_t *src, int softmax_elements,
  ...
  if (softmax_elements == 0) {
    ...
    // launch
   hipLaunchKernelGGL(( kernel), dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
...
bool dispatch_additive_masked_softmax_dropout(
    ...
    hipStream_t streamid) // p is the probability to keep, not drop
{
  ...
  if (softmax_elements == 0) {
    ...
    // launch
   hipLaunchKernelGGL(( kernel), dim3(blocks), dim3(threads), 0, streamid, 
...
bool dispatch_additive_masked_softmax_stream(
    ...
    int pad_batch_stride, hipStream_t streamid) {
  if (softmax_elements == 0) {
    return true;
  } else if (softmax_elements <= 1024) {
    // compute function index. there's a function for each power of two size up
...
void dispatch_masked_scale_softmax_backward_masked_out(
  ...
  if (softmax_elements == 0) {
    ...
    switch (log2_elements) {
    case 0: // 1
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t,
                                                      0, is_log_softmax>)
...
void dispatch_masked_scale_softmax_backward_masked_out_stream(
    ...
    const uint8_t *mask, const uint8_t *pad_mask, acc_t scale,
    int softmax_elements, int softmax_elements_stride, int batch_count,
    int heads, hipStream_t streamid) {
  TORCH_INTERNAL_ASSERT(softmax_elements >= 0 && softmax_elements <= 1024);
  if (softmax_elements == 0) {
    return;
  } else {
    ...
    case 0: // 1
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward_masked_dgrad<input_t, output_t, acc_t,
                                                      0, is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, pad_mask, scale, batch_count,
              softmax_elements_stride, softmax_elements, heads);
      break;
    case 1: // 2
...
bool dispatch_masked_scale_softmax_backward_recompute(
    ...
    int batch_count, hipStream_t streamid) {
...
void dispatch_masked_scale_softmax_backward_stream(
    ...
    int softmax_elements_stride, int batch_count, hipStream_t streamid) {
  TORCH_INTERNAL_ASSERT(softmax_elements >= 0 && softmax_elements <= 1024);
  if (softmax_elements == 0) {
    return;
  } else {
    ...
    switch (log2_elements) {
    case 0: // 1
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 0,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
      ...
      break;
    case 1: // 2
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 1,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
      break;
    case 2: // 4
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 2,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
      break;
    case 3: // 8
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 3,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
      break;
    case 4: // 16
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 4,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
      break;
    case 5: // 32
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 5,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
      break;
    case 6: // 64
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 6,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
      break;
    case 7: // 128
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 7,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
      break;
    case 8: // 256
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 8,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
      break;
    case 9: // 512
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 9,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
      break;
    case 10: // 1024
     hipLaunchKernelGGL(( masked_scale_softmax_warp_backward<input_t, output_t, acc_t, 10,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, streamid, 
              grad_input, grad, output, mask, scale, batch_count,
              softmax_elements_stride, softmax_elements);
...
void dispatch_softmax_backward_fused_native(
  ...
  if (softmax_elements == 0) {
    ...
    switch (log2_elements) {
    case 0: // 1
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 0,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
      ...
      break;
    case 1: // 2
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 1,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
      break;
    case 2: // 4
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 2,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
      break;
    case 3: // 8
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 3,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
      break;
    case 4: // 16
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 4,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
      break;
    case 5: // 32
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 5,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
      break;
    case 6: // 64
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 6,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
      break;
    case 7: // 128
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 7,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
      break;
    case 8: // 256
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 8,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
      break;
    case 9: // 512
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 9,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
      break;
    case 10: // 1024
     hipLaunchKernelGGL(( softmax_warp_backward_fused_native<input_t, output_t, acc_t, 10,
                                         is_log_softmax>)
          , dim3(blocks), dim3(threads), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
              grad_input, grad, output, batch_count, softmax_elements_stride,
              softmax_elements);
