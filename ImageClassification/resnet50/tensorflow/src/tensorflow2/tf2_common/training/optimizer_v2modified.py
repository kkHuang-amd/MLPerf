...
class OptimizerV2Modified(optimizer_v2.OptimizerV2):
  ...
  def _distributed_apply_org(self, distribution, grads_and_vars, name,
                             apply_state):
  ...
  if model_helpers.version_compare_ge(tf.__version__, "2.6.0"):
    ...
    decorator=tf.function(experimental_compile=False)
  @decorator
  def _distributed_apply_compile(self, distribution, grads_and_vars, name,
    ...
    self._distributed_apply_org(distribution, grads_and_vars, name, apply_state)
