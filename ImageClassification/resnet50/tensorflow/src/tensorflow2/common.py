  ...
  if (get_flag_module(flags_obj, 'model') is None or
    ...
    if flags_obj.optimizer == 'SGD':
      ...
      optimizer = gradient_descent_v2.SGD(
          learning_rate=lr_schedule, momentum=flags_obj.momentum)

    elif flags_obj.optimizer == 'LARS':
      use_experimental_compile = True if FLAGS.lars_xla else False
      if  use_experimental_compile==True:
        print("XLA is enabled with LARS")
      else:
        print("XLA is disabled with lARS")
      optimizer = lars_optimizer.LARSOptimizer(
          ...
          epsilon=flags_obj.lars_epsilon, use_experimental_compile=use_experimental_compile)
...
def define_keras_flags(
  ...
  flags.DEFINE_boolean(
          'lars_xla', False,
          'Whether to enable XLA while using LARS optimizer')
