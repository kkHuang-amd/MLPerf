   ...
   __device__ __forceinline__ void operator()(
    ...
    if (is_skipped) {
      ...
      if (g_norm > 0.0f && p_norm > 0.0f) {
        trust_ratio = trust_coefficient * p_norm / (g_norm + p_norm * weight_decay);
      }
        ...
        i_start += blockDim.x*ILP)
    {
	  ...
	  //incoming_moms[ii] = incoming_moms[ii] * momentum + (incoming_grads[ii]);

	  weight_in[i] += incoming_moms[ii];
