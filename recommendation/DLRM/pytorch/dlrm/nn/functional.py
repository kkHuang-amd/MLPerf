...
class DotBaseInteractNoWmma(Function):
    @staticmethod
    def forward(ctx, input, bottom_mlp_output, output_padding_width = 1):
        output = cuda_ext.dotBasedInteractNoWmmaFwd(input, bottom_mlp_output, output_padding_width)
        ctx.save_for_backward(input)
        ctx.output_padding_width = output_padding_width
        return output
    ...
    def backward(ctx, grad_output):
        input, = ctx.saved_tensors
        output_padding_width = ctx.output_padding_width
        grad, mlp_grad = cuda_ext.dotBasedInteractNoWmmaBwd(input, grad_output, output_padding_width)
        return grad, mlp_grad, None
