...
FLAGS = flags.FLAGS
OPTS = tf.io.TFRecordOptions(tf.compat.v1.python_io.TFRecordCompressionType.ZLIB)


def test():
    records = list(tqdm(tf.compat.v1.python_io.tf_record_iterator(FLAGS.out_path, OPTS)))
    print()
    print("Values:")
...
def main(unused_argv):
    ...
    print()
    with tf.io.TFRecordWriter(out_path, OPTS) as writer:
        ...
        with tf.compat.v1.Session() as sess:
            ...
            tf.compat.v1.train.Saver().restore(sess, FLAGS.model)

            # TODO(seth): Add policy entropy.

            p_err = tf.nn.softmax_cross_entropy_with_logits(
                logits=logits,
