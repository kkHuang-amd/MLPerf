...
class ResNetBuilder(object):
    ...
    def conv(self, kernel_size, in_planes, out_planes, name, stride=1):
        ...
        if self.config['nonlinearity'] == 'relu':
            ...
            if self.get_logs and self.local_rank==0:
                log_event(key=constants.WEIGHTS_INITIALIZATION, metadata=dict(tensor=name))
    ...
    def fc(self, planes, num_classes=1000):
        ...
        if self.get_logs and self.local_rank == 0:
            log_event(key=constants.WEIGHTS_INITIALIZATION, metadata=dict(tensor="fc1_bias"))
            log_event(key=constants.WEIGHTS_INITIALIZATION, metadata=dict(tensor="fc1_weight"))
        return fc
