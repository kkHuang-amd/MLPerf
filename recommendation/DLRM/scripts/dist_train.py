...
from dlrm.utils import metrics, optim
import dlrm.nn
...
def main(argv):
    ...
    device_mapping = dist_model.get_criteo_device_mapping(
            world_size, preset_mapping=FLAGS.preset_device_mapping)
    ...
    model = dist_model.DistDlrm(
        ...
        **config,
        ...
        use_apex_mlp=FLAGS.use_apex_mlp,
        compress_embedding=FLAGS.compress_embedding,
        use_fused_emb_sgd=FLAGS.use_fused_emb_sgd,
        use_wmma_interaction=FLAGS.use_wmma_interaction)
    ...
    embedding_sdg = optim.EmbeddingSGD if FLAGS.use_emb_sgd_ext else torch.optim.SGD
    embedding_optimizer = embedding_sdg([
        {'params': model.bottom_model.joint_embedding.parameters(), 'lr': emb_lr},
        ]) if not FLAGS.use_fused_emb_sgd \
           or not isinstance(model.bottom_model.joint_embedding, dlrm.nn.JointSparseEmbedding) \
           else model.bottom_model.joint_embedding
    mlp_params = []
    if len(list(model.bottom_model.bottom_mlp.parameters())) > 0:
        mlp_params.append(
                {'params': model.bottom_model.bottom_mlp.parameters(), 'lr': scaled_lrs[0]})
    else:
        scaled_lrs = scaled_lrs[1:]
    mlp_params.append({'params': model.top_model.parameters(), 'lr': scaled_lrs[-1]})
    mlp_optimizer = apex_optim.FusedSGD(
            mlp_params, momentum=FLAGS.mlp_momentum, dampening=FLAGS.mlp_momentum)
    ...
    mlperf_logger.log_event(key='gradient_accumulation_steps', value=1)

    lr_scheduler = utils.LearningRateScheduler(optimizers=[mlp_optimizer, embedding_optimizer],
                                               base_lrs=[scaled_lrs, [emb_lr]],
    ...
    num_vectors = sum(vectors_per_gpu)
    ...
    for epoch in range(FLAGS.epochs):
        ...
        for step, (numerical_features, categorical_features, click) in enumerate(
            ...
            if click.shape[0] != FLAGS.batch_size: # last batch
                ...
                if categorical_features is not None:
                    if FLAGS.compress_embedding:
                        feature = categorical_features[0]
                        padding_categorical = torch.ones(
                            padding_size, feature.shape[1],
                            device=feature.device, dtype=feature.dtype)
                        categorical_features = [torch.cat((feature, padding_categorical), dim=0) \
            ...
            if FLAGS.compress_embedding:
                # Transfer metadata
                (categorical_features, num_unique_indices, num_unique_indices_handles,
                        inverse_indices, inverse_indices_handle) = \
                                dist_model.send_compression_metadata(
                                        categorical_features,
                                        model.bottom_model.embedding_offsets,
                                        batch_size_per_gpu,
                                        config['embedding_dim'],
                                        num_vectors, vectors_per_gpu,
                                        device_mapping['bottom_mlp'],
                                        FLAGS.device)
            ...
            if FLAGS.compress_embedding:
                from_bottom = dist_model.bottom_to_top(
                    batch_size_per_gpu, config['embedding_dim'], device_mapping,
                    FLAGS.use_alltoall_base, num_unique_indices, num_unique_indices_handles,
                    inverse_indices, inverse_indices_handle, *bottom_out)
            else:
                from_bottom = dist_model.bottom_to_top(
                    bottom_out, batch_size_per_gpu, config['embedding_dim'], vectors_per_gpu,
                    FLAGS.use_alltoall_base)
            ...
            param_groups = mlp_optimizer.param_groups if FLAGS.use_fused_emb_sgd \
                           else itertools.chain(embedding_optimizer.param_groups, mlp_optimizer.param_groups)
            ...
            elif FLAGS.debug and step % print_freq == 0:
            ...
            if global_step % test_freq == 0 and global_step > 0 and global_step / steps_per_epoch >= FLAGS.test_after:
                ...
                if auc > FLAGS.auc_threshold:
                    ...
                    mlperf_logger.log_end(key=mlperf_logger.constants.EPOCH_STOP,
                                          metadata={mlperf_logger.constants.EPOCH_NUM: math.ceil(mlperf_epoch_index)})
        ...
        mlperf_logger.barrier()
        mlperf_logger.log_end(key=mlperf_logger.constants.EPOCH_STOP,
                              metadata={mlperf_logger.constants.EPOCH_NUM: epoch + 2})
...
def dist_evaluate(model, data_loader, data_cache):
    ...
    compute_auc = world_size > 4 or rank == 0
    ...
    with torch.no_grad():
        ...
        for step, (numerical_features, categorical_features, click) in enumerate(eval_data_iter):
            ...
            if click.shape[0] != FLAGS.test_batch_size: # last batch
                ...
                if categorical_features is not None:
                    if FLAGS.compress_embedding:
                        feature = categorical_features[0]
                        padding_categorical = torch.ones(
                            padding_size, feature.shape[1], device=feature.device, dtype=feature.dtype)
                        categorical_features = [torch.cat((feature, padding_categorical), dim=0) \
                                                for feature in categorical_features]
                    else:
                        padding_categorical = torch.ones(
            ...
            if FLAGS.compress_embedding:
                from_bottom = dist_model.bottom_to_top(
                        batch_size_per_gpu, model.embedding_dim,
                        device_mapping, FLAGS.use_alltoall_base,
                        num_unique_indices, num_unique_indices_handles,
                        inverse_indices, inverse_indices_handle, *bottom_out)
            else:
                from_bottom = dist_model.bottom_to_top(
                        bottom_out, batch_size_per_gpu, model.embedding_dim,
                        vectors_per_gpu, FLAGS.use_alltoall_base)

            output = model.top_model(from_bottom).squeeze()

            buffer_dtype = torch.float32 if not FLAGS.fp16 else torch.float16

            # TODO: Use gather for world size <= 4 to reduce the communication
            # Note that gather is currently not supported in PyTorch
            ...
            if FLAGS.debug and step % print_freq == 0 and step != 0:
                torch.cuda.synchronize()
        ...
        if compute_auc:
            ...
            auc = torch.tensor(0, dtype=torch.float32, device=FLAGS.device)

        if world_size <= 4:
            torch.distributed.broadcast(auc, src=0)

        del output
        del click
        del output_receive_buffer
        if compute_auc:
            del y_true
            del y_score
