...
log_event(key=constants.CACHE_CLEAR, value=True, stack_offset=0)"
        ...
        # Run experiment
        # docker exec -it "${_config_env[@]}" "${_cont_name}" rocprof --stats -o /logs/output_20221018.csv ./run_and_time.sh
        #docker exec -it "${_config_env[@]}" "${_cont_name}" rocprof --stats -o /logs/output_20221016.csv ./run_and_time.sh
