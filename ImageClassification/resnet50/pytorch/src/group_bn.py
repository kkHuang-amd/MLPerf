...
class BatchNorm2d_NHWC(_BatchNorm):
    ...
    def __init__(self, num_features, fuse_relu=False, bn_group=1, torch_channels_last=False, local_rank=0, get_logs=False, name_beta=None, name_gamma=None, max_cta_per_sm=2, cta_launch_margin=12, multi_stream=False):
        ...
        self.name_beta=name_beta
        self.name_gamma=name_gamma
        if self.get_logs and self.local_rank == 0:
            log_event(key=constants.WEIGHTS_INITIALIZATION, metadata=dict(tensor=self.name_beta))
            log_event(key=constants.WEIGHTS_INITIALIZATION, metadata=dict(tensor=self.name_gamma))
