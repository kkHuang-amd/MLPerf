...
def batched_reader(file_path):
    reader = tf.compat.v1.python_io.tf_record_iterator(file_path, OPTS)
...
def compare(pair):
    ...
    for a, b in compare_iter:
        ...
        xa, xb, ra, rb = tf.compat.v1.Session().run([xa, xb, ra, rb])
...
def main(remaining_argv):
    paths = sorted(tf.io.gfile.listdir(FLAGS.in_dir))
    total = len(paths)
    pairs = []
