...
def batch_parse_tf_example(batch_size, layout, example_batch):
    ...
    parsed = tf.io.parse_example(serialized=example_batch, features=features)
...
def get_input_tensors(batch_size, feature_layout, tf_records, num_repeats=1,
    ...
    dataset = read_tf_records(
        ...
        interleave=False)
    dataset = dataset.filter(lambda t: tf.equal(tf.shape(input=t)[0], batch_size))
    dataset = dataset.map(
        functools.partial(batch_parse_tf_example, batch_size, feature_layout), num_parallel_calls=8)
    if random_rotation:
