...
class UpdateRatioSessionHook(tf.estimator.SessionRunHook):
    ...
    def begin(self):
        ...
        self.file_writer = tf.compat.v1.summary.FileWriterCache.get(self.output_dir)
...
def init_train(rank, tcomm, model_dir):
    ...
    # Training
    return dual_net.DualNetwork(os.path.join(FLAGS.work_dir, 'model.ckpt-5672'))
...
def run(state, rank, dual_net_model, num_examples, tf_records):
    # calculate steps
    ...
    dataset = preprocessing.get_input_tensors(
        ...
        FLAGS.input_layout, tf_records, filter_amount=FLAGS.filter_amount,
        shuffle_examples=FLAGS.shuffle_examples,
        shuffle_buffer_size=FLAGS.shuffle_buffer_size, random_rotation=True)
    ...
    for step in range(steps):
        try:
            ...
            dual_net_model.train_step(features, labels, step==0)
        except tf.errors.OutOfRangeError:
            break

    # export graph
    if rank == 0:
        model_path = dual_net_model.save_checkpoint(state.iter_num)
        if FLAGS.freeze:
