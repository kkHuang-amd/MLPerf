...
def _histogram_move_keys_by_game(sess, ds, batch_size=8*1024):
    ...
    ds = ds.map(lambda x: tf.strings.substr(input=x, pos=0, len=12))
    iterator = tf.compat.v1.data.make_initializable_iterator(ds)
    sess.run(iterator.initializer)
    get_next = iterator.get_next()
...
def _game_keys_as_array(ds):
    ...
    ds = ds.map(lambda x:
                tf.strings.to_number(tf.strings.substr(input=x, pos=2, len=10) +
                                     '.' +
                                     tf.strings.substr(input=x, pos=15, len=3),
                                     out_type=tf.float64))
...
def count_elements_in_dataset(ds, batch_size=1*1024, parallel_batch=8):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        iterator = tf.compat.v1.data.make_initializable_iterator(dsc)
        sess.run(iterator.initializer)
        get_next = iterator.get_next()
        counted = 0
        try:
            while True:
                ...
                counted = sess.run(tf.reduce_max(input_tensor=get_next)) + 1
