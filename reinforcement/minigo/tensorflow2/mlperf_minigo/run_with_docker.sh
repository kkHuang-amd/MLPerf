...
: "${CLEAR_CACHES:=1}"
: "${DATADIR:=/global/home/mtsai/data/minigo}"
#: "${DATADIR:=/data/minigo_data_19x19}"
: "${LOGDIR:=$(pwd)/results}"
...
docker run --rm --init --detach \
    --device=/dev/kfd --device=/dev/dri --group-add video --cap-add=SYS_PTRACE --shm-size=64G \
...
# Copy (updated) config file - to avoid rebuilding docker for new configs
docker cp ${_config_file} "${_cont_name}:/opt/reinforcement/${_config_file}"
