...
def parse_args():
    ...
    parser.add_argument("--nnuma_nodes", type=int, required=True,
...
def main():
    ...
    NGPUS_PER_SOCKET = max(1,args.nproc_per_node // args.nsockets_per_node)
    NCORES_PER_GPU = args.ncores_per_socket // NGPUS_PER_SOCKET

    NNUMA_NODES = args.nnuma_nodes
    NGPUS_PER_NNODE = max(1,args.nproc_per_node // args.nnuma_nodes)
    ...
    for local_rank in range(0, args.nproc_per_node):
        ...
        if not args.no_membind:
            memnode = local_rank // NGPUS_PER_NNODE
            numactlargs += [ "--preferred={}".format(memnode) ]
