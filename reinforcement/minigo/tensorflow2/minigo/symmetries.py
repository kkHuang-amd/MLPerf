...
def rotate_train_nhwc(x, pi):
    ...
    def rotate(tensor):
        ...
        tensor = tf.compat.v1.where(
            ...
            tf.transpose(a=tensor, perm=[1, 0, 2]),
            tensor)
        return tensor

    # TODO(tommadams): use tf.ensure_shape instead of tf.assert_equal.
    squares = go.N * go.N
    assert_shape_pi = tf.compat.v1.assert_equal(pi.shape.as_list(), [squares + 1])

    x_shape = x.shape.as_list()
    assert_shape_x = tf.compat.v1.assert_equal(x_shape, [go.N, go.N, x_shape[2]])
...
def rotate_train_nchw(x, pi):
    ...
    def rotate(tensor):
        ...
        tensor = tf.compat.v1.where(
            tf.bitwise.bitwise_and(sym, 4) > 0,
            tf.transpose(a=tensor, perm=[0, 2, 1]),
            tensor)
        return tensor
    ...
    x_shape = x.shape.as_list()
    assert_shape_x = tf.compat.v1.assert_equal(x_shape, [x_shape[0], go.N, go.N])
