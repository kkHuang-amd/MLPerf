...
def get_data_loader(dataset_path,
    ...
    if dataset_type == 'bin':
        ...
        dataset_train = DistCriteoDataset(
            os.path.join(dataset_path, "train"), batch_size=batch_size, shuffle=shuffle,
                         compress_embedding=compress_embedding, **kwargs)
        ...
        dataset_test = DistCriteoDataset(
            os.path.join(dataset_path, "test"), batch_size=test_batch_size,
                         compress_embedding=compress_embedding, **kwargs)
    ...
    data_loader_args = dict(
        ...
        pin_memory=False,
        collate_fn=functools.partial(data_collate_fn, device=return_device, orig_stream=torch.cuda.current_stream(), compress_embedding=compress_embedding))
...
class DistCriteoDataset(Dataset):
    ...
    def __init__(self, data_path, batch_size=1, shuffle=False, numerical_features=False, categorical_features=None,
                 prefetch_depth=10, compress_embedding=False):
...
def data_collate_fn(batch_data, device="cuda", orig_stream=None, compress_embedding=False):
    ...
    if orig_stream is not None:
        ...
        if categorical_features is not None:
            if compress_embedding:
                for tensor in categorical_features:
