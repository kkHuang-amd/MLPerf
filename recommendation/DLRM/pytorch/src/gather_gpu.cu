...
__global__ void GatherFP16Kernel(const float* params,
                             int64_t num_features,
                             int embed_size,
                             int batch_size,
                             int query_nnz,
...
#pragma unroll
  for (int i = 0; i < query_nnz; ++i) {
    // printf("%d, %d, %d\n", bid, i, shmem_indices[i]);
    ret[(bid * query_nnz + i) * embed_size + tid] =
        __float2half(params[(int64_t)shmem_indices[i] * embed_size + tid]);
  }
}
...
__global__ void OneHotFP16Kernel(const float* params,
                                 int64_t num_features,
                                 int embed_size,
                                 int batch_size,
                                 const int64_t* indices,
  ...
  ret[bid * embed_size + tid] = __float2half(params[(int64_t)indices[bid] * embed_size + tid]);
}
...
torch::Tensor gather_gpu_fwd(const torch::Tensor weight, const torch::Tensor indices, const bool to_fp16) {
  ...
  auto outputs =
      torch::empty(batch_size * query_nnz * embed_size, at::device(at::kCUDA).dtype(to_fp16 ? at::kHalf : at::kFloat));

  if (query_nnz != 1) {
    if (to_fp16) {
      GatherFP16Kernel<<<batch_size,
                         embed_size,
    ...
    if (to_fp16) {
      OneHotFP16Kernel<<<batch_size, embed_size, 0, at::cuda::getCurrentCUDAStream()>>>(
